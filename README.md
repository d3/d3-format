...
Yet rounding error is not the only reason to customize number formatting. When comparing numbers, you want them to be formatted consistently; so above, 0.0 would be better than 0. Grouping thousands improves the legibility of large numbers (e.g., 42,000) without resorting to technical notation (4.2e+4). Currencies often need fixed precision ($3.50). Rounding to significant digits (4021 ↦ 4000) may avoid exaggerating accuracy when reporting results. And different cultures have different conventions for formatting numbers: some prefer 42.000,00 to 42,000.00.
