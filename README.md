...
Rounding error is not the only reason to customize number formatting. A table of numbers should be formatted consistently for comparison; so above, 0.0 would be better than 0. Large numbers should have grouped digits (e.g., 42,000) or be displayed in scientific notation (4.2e+4). Currencies should have fixed precision ($3.50). Reported numerical results should be rounded to significant digits (4021 ↦ 4000) to indicate precision. And numbers should be formatted appropriately to the reader’s locale (42.000,00 or 42,000.00).
